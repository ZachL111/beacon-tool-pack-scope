...
`beacon-tool-pack-scope` is centered on cli tools work in Python. The technical target is: Package a Python local lab for pack analysis with log and snapshot fixtures, replay consistency checks, and documented operating limits.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 4.
